...
test('real HTTP/WS: shared state, role exclusion, reconnect takeover, release and rooms', async t => {
  ...
  assert.equal((await fetch(`${base}/teacher.html`)).status, 200);
  assert.equal((await fetch(`${base}/qa.html`)).status, 200);
  assert.equal((await fetch(`${base}/about.html`)).status, 200);
  assert.equal((await fetch(`${base}/journal.html`)).status, 200);
  assert.equal((await fetch(`${base}/parents.html`)).status, 200);
  assert.equal((await fetch(`${base}/archive.css`)).status, 200);
  ...
  assert.equal((await fetch(`${base}/api/lan`)).status, 200);
  assert.equal((await fetch(`${base}/api/qr`)).status, 200);
  assert.equal((await fetch(`${base}/server.mjs`)).status, 404);
  assert.equal((await fetch(`${base}/simulation.mjs`)).status, 404);
